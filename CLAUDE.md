...
- **Two ways to run the app locally:** `npm run dev` → real production DB (`.env.local`). `npm run dev:test` → disposable `drink-tracker-test` DB (`.env.test`). Use `dev:test` for anything exploratory/destructive; reach for plain `dev` only when you deliberately need to look at real data.
